...
Roslyn-suspect compile failure fallback: when overlay compile reports an error that the Roslyn picture does not explain (for example `find_references` returned empty but overlay flags an undefined reference in another file, or a consumer site Roslyn never surfaced is reported), drop to direct filesystem tools (Read/Grep/Glob) to investigate the watched tree. The intended safety model is layered: Roslyn discovery is the first pass, overlay compile validation is the explicit catch for incomplete scope (including Roslyn-fallibility and Codex-style agents that lack Roslyn entirely), vote-plus-hash plus WinMerge is the catch for operator/byte mismatch. Each layer assumes the one above can be fallible. When overlay trips and Roslyn looks incomplete, diagnosing via filesystem reads is the intended path, not a Roslyn-first violation. Re-stage with the corrected scope (typically: add the missed file to the same `sessionId` before re-launching review). Operator-granted authority; not a general bypass of Roslyn-first read discipline for routine discovery.
