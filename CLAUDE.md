...
Claude should bind only the Monitor MCP server from this repository's `.mcp.json`. Roslyn/CodeLens capability is reached through the Monitor-owned index, source-map, and hub-backed tooling surfaces; do not expect a separate Claude-facing `roslyn-codelens` MCP namespace in the normal VS Code workflow.

When using any Roslyn-derived tooling, follow the active tool manifest and `Docs/RoslynToolingTeachingSpec.md` where applicable for exact argument names, argument acquisition, tool recipes, and negative examples. Do not guess argument names.

Always prefer Monitor/Roslyn-derived tools over text or grep search for C# symbol discovery. Never edit watched source directly; all watched-source changes must go through System Monitor staging. Use diagnostics after staged edits compile to confirm no new errors before requesting Operator review.

Before emitting a call site to a type reached through `using` directives, local namespace context, or a known dependency, load that target type's real callable surface first. Use Monitor solution-index/source-map tools such as `find_indexed_symbols`, `query_solution_index`, `get_source_map`, and `get_symbol` when the target is in the watched project. Write calls against actual method names, return types, parameter types, and overloads. Overlay compile validation is the safety floor for missed call-site context, not a substitute for loading the target surface.
