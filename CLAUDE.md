...
Mid-iteration refinement of an active candidate may use the harness `Edit` tool directly on the Working mirror file (path: `Working\<observedRootKey>\<relative source path>`). This is only valid while a candidate state is active — after a prior `submit_file` / composition call and before `record_diff_decision` cleans up the candidate state. Once `record_diff_decision(accepted|rejected)` runs, the candidate state JSON is removed and `stage_candidate_for_review` will fail on a follow-up Edit-only attempt; the next edit must start with a `submit_file` (or other composition call) to re-establish a candidate. Default composition tools (`submit_file`, `submit_symbol`, `add_method`, etc.) remain the primary path. Telemetry caveat: the candidate state JSON (operationCount, intermediate syntaxValidation) will not reflect the Edit until stage time recomputes the hash. For the common "small edit in a large Razor file after a recent accept" case there is no fast path today — file a finding rather than improvising.
