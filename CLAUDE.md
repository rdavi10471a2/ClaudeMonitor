...
5. Use `get_file` only when index/source-map/symbol context is not enough and the file is not large. **Large file threshold: 32KB.** For any file at or above 32KB, call `refresh_file` first and chunk-read the returned Working file path instead of asking MCP to return the whole file. This threshold sits below the empirically observed pass/fail boundary for inline tool results (~36KB pass, ~86KB fail) with margin for JSON envelope overhead.
